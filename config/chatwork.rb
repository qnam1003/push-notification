...
class Chatwork
  ...
  @@API_BASE = 'https://chatwork.bap.jp/api/v1'
  WEBHOOK_URL = 'https://chatwork.bap.jp/hooks/4zBYZ7QYdpgiysQdG/fu38qucJccHqNnG67WctCa3ZcLzgc44agPzG7uyWjxouSsRc'
  API_TOKEN = '4zBYZ7QYdpgiysQdG/fu38qucJccHqNnG67WctCa3ZcLzgc44agPzG7uyWjxouSsRc'
  # tokenを指定してオブジェクトを生成
  ...
  # room_id: 対象のroomID
  # body:    投稿する本文
  def sendMessage(room_id, text)
    
    url = '/chat.postMessage'
    res = createHttpObject(url, :post, {:text => text, channel: "##{room_id}"})
    return res.body ? JSON.parse(res.body) : []
  end
  ...
  private
    ...
    def createRequestObject(method, uri)
      ...
      request_object["X-Auth-Token"] = "R3BskZ-5ZDlf64p_2-eGLOrB0AXVdTt9cBvgE3VsIif"
      request_object["X-User-Id"] = "YpqLvxXuwP3qpDscq"
